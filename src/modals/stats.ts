...
export default class Stats extends Phaser.GameObjects.Image {
  ...
  updateStats(newCounterValue: number) {
    console.log(`Nuevo valor del contador: ${newCounterValue}`);
    // Actualiza tus estadísticas aquí usando newCounterValue
  }
  ...
  showStats(myMaxStat: JSON) {
    if (myMaxStat) {
      ...
      // Set possible colors for touchedMarkers
      ...
      // Set possible colors for untouchedMarkers
